namespace WPF_Kassörskan_V2
{
    ...
    using WPF_Kassörskan_V2.ViewModel;
    #endregion
    ...
    public class ServerConnection
    {
        ...
        /// </summary>
        private async void ResolveServerMessage(byte[] Req)
        {
            // Convert the bytes
            string RequestString = Encoding.Default.GetString(Req);

            if (RequestString.Equals("[ORDERDONE]"))
            {
                UpdateAllOrders();
            }
        }

        private async void UpdateAllOrders()
        {
            CashierViewModel.Instance.DataBaseList = await CashierViewModel.Instance.rep.GetOrderByStatus(2);
            CashierViewModel.Instance.OrderList = new ObservableCollection<OrderModel>();

            foreach (Order item in CashierViewModel.Instance.DataBaseList)
            {
                ...
                CashierViewModel.Instance.OrderList.Add(model);
            }
        }
        ...
        private void ConnectCallback(IAsyncResult ar)
        {
            try 
            {
                ...
                Connection.Send(Encoding.Default.GetBytes("2"));
